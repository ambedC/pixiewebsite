...
// Button Component
export const GlassButton: React.FC<{ children: React.ReactNode; onClick?: () => void; type?: "button" | "submit" | "reset"; disabled?: boolean }> = ({
  children,
  onClick,
  type = "button",
  disabled = false
}) => (
  <button type={type} onClick={onClick} disabled={disabled} className="w-fit">
    <GlassEffect
...
export const GlassCard: React.FC<{ children: React.ReactNode; className?: string }> = ({
  ...
  <GlassEffect
    className={`rounded-[2.5rem] p-8 md:p-12 ${className}`}
  >
    {children}
  </GlassEffect>
);
